...
#### 1. Что такое мат.ожидание?  
  ...
  Более сложный - число возле которого групприруется среднее арифмитическое реализовавшихся значений  
  случ. вел., при большом числе испытаний.  
...
#### 11. В каком случае не совпадают среднее арифметическое, мода и медиана?
  ...
  Левостороннее (хвост вытянут влево) отрицательное распределение среднее меньше медианы, медиана меньше моды.   
  Правостороннее (хвост справа) положительное распределение - мода меньше медианы, медиана меньше среднего.  
...
- стандартная ошибка среднего будет меньше стандартного отклонения ген. сов., в корень из n раз.  
...
Надо подумать  (в плане нормальности выборочных средних) 
Чтобы применить ЦПТ, необходимо выполнить четыре условия:  
...
* ценовых моделей
* путей клиента
* алгоритмов (ранжирование в поисковой выдаче)
...
Распространенные ошибки:  
  ...
  10 Плохой рандомайзер  
  11 Ошибки в сплитовании  
    
#### 2. Что такое Pvalue?

  Pvalue - вероятность получить такие же или более экстремальные значения, при верности  
  нулевой гипотезы (H0).  
  Мера доказательности против нулевой гипотезы - чем pvalue меньше тем сильнее док-во против H0.  
  (Роман с Data Science)  
  Наименьший уровень значимости, при котором мы можем отвергнуть H0, для данной реализации выборки.  
  (ШАД)  
  **Чем не является Pvalue?**  
  Уровнем значимости, реальным уровнем значимости, вероятностью ошибки 1 рода - перечисленное не зависит от выборки.  
  Вероятностью Н0, вероятностью Н0 при условии данной выборки - в фреквнтистком подходе мы не оцениваем вероятность  
  верности Н0. Мы или отвергаем её или не имеем оснований отвергнуть.  
...
#### 3. Как оно распределено когда есть эффект? А когда эффекта нет (синтетический А/А)? 
  ...
  Расределение Pvalue равномерно и случайно в ситуации когда нет эффекта.  
...
#### 7. Каого размера выборка(группа) нужна?
  ...
  * Дисперсии
  ...
  Формула для расчёта выборки здесь.  

#### 8. Как долго проводить эксперимент?

  Узнать у бизнеса сколько есть времени.  
  Для уменьшения факторов - эффекта новизны, сезонности и других следует использовать продолжительность  
  кратную числу недель.

#### 9. Почему нельзя запустить эксперимент и ждать пока из вариантов не будет значимее?
  
  Если кратко то - уровень ошибки 1 рода будет очень большим.  
  
#### Что такое статистический критерий?

   Математическое правило, в соответствии с которым принимается или отвергается та или иная статистическая гипотеза  
   с заданным уровнем значимости.  
   
#### 11. Какие стат.критерии вы знаете?

  Параметрические:
  * семейство t-критериев (Стьюдент, Велч, для одной выборки, двух (зависимой/независимой))
  * ANOVA/MANOVA
  * F критерий Фишера
...
#### 1. Что такое случайная величина?
  ...
  СВ - переменная величина, которая в результате испытания принимает одно из множнества своих возможных  
  значений, причём заранее неизвестно какое именно.
  
#### 2. Что такое функция распределения случайной величиы?
  Так же носит название интегрального закона распределения.  
  Это функция аргумента x которая при каждом x задаёт вероятность того что СВ примет значение  
  меньшее x.  
  Функция, характеризующая распределение случайной величины. вероятность того, что случайная величина  
  X  примет значение, меньшее или равное x, где x — произвольное действительное число.  
  
#### 3. Что такое плотность распределения случайной величины?
  Дифференциальный закон распределения -  это производная функции распределения.  
  
  Плотностью распределения СВ называется функция аргумента Х, которая при каждом Х равна пределу отношения  
  вероятности попадания данной СВ на интервал дельта(х) в окрестности Х к длине этого интервала, когда длина  
  стремится к 0.  

  Вещественная функция, характеризующая сравнительную вероятность реализации тех или иных значений случайной переменной  
